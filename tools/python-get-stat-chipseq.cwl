  ...
  - class: InlineJavascriptRequirement
    expressionLib:
      - var get_output_filename = function() { if (inputs.output_filename) { return
        inputs.output_filename; } var root = inputs.bowtie_log.basename.split('.').slice(0,-1).join('.');
        var ext = ".stat"; return (root == "")?inputs.bowtie_log.basename+ext:root+ext;
        };
      - var get_formatted_output_filename = function() { if (inputs.formatted_output_filename)
        { return inputs.formatted_output_filename; } var root = inputs.bowtie_log.basename.split('.').slice(0,-1).join('.');
        var ext = "_stats.tsv"; return (root == "")?inputs.bowtie_log.basename+ext:root+ext;
        };
hints:
