...
inputs:
  ...
  norm_control_to_treatment:
    ...
    doc: "Two options, “norm” denotes normalization of control to\ntreatment data,
      “non” skips this behavior. \"norm\" is \necommended unless experimental and
...
doc: "Tool runs peak calling using SEACR_1.3.sh command. When running in stringent
  mode\n(default), if zero peaks are called from your input data there will be no
  output\nbed file, and the tool script will fail.\n\nSEACR: Sparse Enrichment Analysis
  for CUT&RUN and CUT&TAG sequencing library data.\n        Input bedGraph may be
  from processed single or paired-end library data\n\n  Usage: bash SEACR_1.3.sh <experimental
  bedgraph>.bg [<control bedgraph>.bg | <FDRthreshold>] [norm | non] [relaxed | stringent]
  output prefix\n\n  Description of input fields:\n\n      Field 1:    Target data
  bedgraph file in UCSC bedgraph format\n                  (https://genome.ucsc.edu/goldenpath/help/bedgraph.html)\n\
  \                  that omits regions containing 0 signal.\n      Field 2:    Control
  (IgG) data bedgraph file to generate an empirical\n                  threshold for
  peak calling. Alternatively, a numeric\n                  threshold n between 0
  and 1 returns the top n fraction of\n                  peaks based on total signal
  within peaks.\n      Field 3:    “norm” denotes normalization of control to target
  ...
  uses a total signal threshold between the knee\n                  and peak of the
  total signal curve, and corresponds to the\n                  “relaxed” mode described
  in the text, whereas “stringent”\n                  uses the peak of the curve,
  and corresponds to “stringent”\n                  mode.\n      Field 5:    Output
  prefix\n\n  Output file:\n  <output prefix>.<mode>.bed (Bed file of enriched regions)\n\
  \n  Output data structure: \n  <chr>\t<start>\t<end>\t<AUC>\t<max signal>\t<max
  signal region>\n\n  Description of output fields:\n  Field 1: Chromosome\n  Field
  2: Start coordinate\n  Field 3: End coordinate\n  Field 4: Total signal contained
  within denoted coordinates\n  Field 5: Maximum bedgraph signal attained at any base
  pair within denoted coordinates\n  Field 6: Region representing the farthest upstream
  and farthest downstream bases within\n            the denoted coordinates that are
  represented by the maximum bedgraph signal\n\n  Examples:\n  bash SEACR_1.3.sh target.bedgraph
  IgG.bedgraph norm relaxed output\n  Calls enriched regions in target data using
  normalized IgG control track with relaxed threshold\n\n  bash SEACR_1.3.sh target.bedgraph
  IgG.bedgraph non relaxed output\n  Calls enriched regions in target data using non-normalized
  IgG control track with relaxed threshold\n  bash SEACR_1.3.sh target.bedgraph 0.01
  non relaxed output\n  Calls enriched regions in target data by selecting the top
  1% of regions by area under the curve (AUC)\n\n\nReferences:\n    Link to SEACR
  publication: https://doi.org/10.1186/s13072-019-0287-4\n    Link to SEACR github
  repo: https://github.com/FredHutch/SEACR\n        \"To address the problem of oversensitivity
  in CUT&RUN peak calling, we\n        developed Sparse Enrichment Analysis for CUT&RUN
  (SEACR), a peak calling\n        algorithm that enforces precision from sparse data
  by quantifying the\n        global distribution of background signal and using it
  to set a stringent\n        empirical threshold for peak identity.\"\n\n  Scaling
  factor based on the following publication:\n      Meers MP, Tenenbaum D, Henikoff
  S. (2019). Peak calling by Sparse Enrichment\n      Analysis for CUT&RUN chromatin
