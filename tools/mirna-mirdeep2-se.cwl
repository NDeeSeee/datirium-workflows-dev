...
doc: "A CWL tool for discovering known or novel miRNAs from deep sequencing data using
  the miRDeep2 tool.\nCalls the shell wrapper for the SciDAP miRDeep2 pipeline. The
  ExoCarta exosome database is also used\nfor identifying exosome-related miRNAs,
  and TargetScan's organism-specific databases are used for\nidentifying miRNA gene
  targets.\n\nPrimary Output files:\n- mirs_known.tsv, known mature miRNAs detected
  by mirdeep2, \"Known miRNAs\" tab\n- mirs_novel.tsv, known novel miRNAs detected
  by mirdeep2, \"Novel miRNAs\" tab\nSecondary Output files:\n- mirs_known_exocarta_deepmirs.tsv,
  list of detected miRNA also in ExoCarta's exosome database, \"Detected Exosome miRNAs\"\
  \ tab\n- mirs_known_gene_targets.tsv, pre-computed gene targets of known mature
  mirs, downloadable\n- known_mirs_mature.fa, known mature mir sequences, downloadable\n
  - known_mirs_precursor.fa, known precursor mir sequences, downloadable\n- novel_mirs_mature.fa,
  novel mature mir sequences, downloadable\n- novel_mirs_precursor.fa, novel precursor
  mir sequences, downloadable\nReports:\n- overview.md (input list, alignment & mir
  metrics), \"Overview\" tab\n- mirdeep2_result.html, summary of mirdeep2 results,
  \"miRDeep2 Results\" tab\n\nPARAMS:\n  -h\thelp\tshow this message\n  -t  INT\t\t
  number of threads\n  -b  DIR\t\tpath to bowtie2 indices folder of genome reference\n\
  \  -r  FILE\tpath to genome reference FASTA file\n  -g  STRING\tgenome short name
  (hg19, hg38, mm10, rn7, dm3)\n  -a  STRING\tsequencing adapter for clipping reads
  (default: TCGTAT)\n  -f  FILE\tpath to sequence read FASTQ file\n\nGENOME:\nFor
  filtering mirbase by organism.\n  genome\t#organism   #division   #name   \t\t\t\
  \t  #tree   \t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t                               \
  ...
  miRNA candidates, the following may be used as a filtering guideline:\n1. miRDeep
  score > 4 (some authors use 1)\n2. not present a match with rfam\n3. should present
  a significant RNAfold (\"yes\")\n4. a number of mature reads > 10\n5. if applicable,
  novel mir must be expressed in multiple samples\n\n____________________________________________________________________________________________________\n
