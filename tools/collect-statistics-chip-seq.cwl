...
inputs:
  script:
    ...
    default: "#!/usr/bin/env python3\nimport os\nimport sys\nimport argparse\nimport
      ...
      --trim1\",           help=\"Path to Trimgalore report file for FASTQ 1. Optional\"\
      , required=False)\n    general_parser.add_argument(\"--trim2\",           help=\"\
      Path to Trimgalore report file for FASTQ 2. Optional\", required=False)\n  \
      \  general_parser.add_argument(\"--bowtie\",          help=\"Path to Bowtie
      report file\",                           required=True)\n    general_parser.add_argument(\"\
      --bamstats\",        help=\"Path to bam statistics report file\",          \
      \         required=True)\n    general_parser.add_argument(\"--bamstatsfilter\"\
      ,  help=\"Path to bam statistics report file after filtering\",   required=True)\n\
      \    general_parser.add_argument(\"--macs2\",           help=\"Path to MACS2
      called peaks xls file\",                  required=True)\n    general_parser.add_argument(\"\
      --atdp\",            help=\"Path to ATDP output TSV file\",                \
      \         required=True)\n    general_parser.add_argument(\"--preseq\",    \
      \      help=\"Path to Preseq output file\",                           required=False)\n\
      \    general_parser.add_argument(\"--paired\",          help=\"Process as paired-end.
      Default: False\",                action=\"store_true\")\n    general_parser.add_argument(\"\
      --output\",          help=\"Output filename prefix\",                      \
      \         required=True)\n    return general_parser\n\n\ndef normalize_args(args,
      skip_list=[]):\n    normalized_args = {}\n    for key,value in args.__dict__.items():\n\
      \        if key not in skip_list:\n            normalized_args[key] = value
      if not value or os.path.isabs(value) else os.path.normpath(os.path.join(os.getcwd(),
      ...
      = res_function(int(res_function(value)/2))\n                else:\n        \
      \            collected_results[header][res_key] = res_function(value)\n    \
      \    except Exception:\n            pass\n    if key_dict.get(\"order\", None):\n\
      \        collected_results[header] = {k: collected_results[header][k] for k
      in key_dict[\"order\"] if k in collected_results[header]}\n\n\ndef process_macs2_xls(filepath,
      ...
      prev_end, prev_chr = start, end, chr\n    collected_results[header][\"number
      of peaks called\"] = count\n    collected_results[header][\"mean peak size\"\
      ] = round(float(length)/float(count), 2)\n    in_treatment = collected_results[header][\"\
      reads/pairs after filtering in treatment\"]\n    mapped = collected_results[\"\
      BAM statistics after filtering\"][\"reads/pairs mapped\"]\n    collected_results[header][\"\
      fraction of reads in peaks\"] = round(float(in_treatment)/float(mapped),2)\n\
      \    if MACS2.get(\"order\", None):\n        collected_results[header] = {k:
      collected_results[header][k] for k in MACS2[\"order\"] if k in collected_results[header]}\n
      \n\ndef process_atdp_results(filepath, collected_results, header):\n    if not
      collected_results.get(header, None):\n        collected_results[header] = {}\n\
      \    collected_results[header][\"maximum\"] = str(pandas.read_csv(filepath,
      sep=\"\\t\")[\"Y\"].max())\n\n\ndef process_preseq_results(filepath, collected_results,
      header, threashold=0.001):\n    px, py = 0, 0\n    for line in open_file(filepath):\n\
      \        if \"TOTAL_READS\" in line:\n            continue\n        values =
      [float(l.strip()) for l in line.strip().split()]\n        dx, dy = values[0]-px,
      values[1]-py\n        px, py = values[0], values[1]\n        if dx != 0:\n \
      \           angle = math.degrees(math.atan2(dy, dx))\n            if angle <=
      threashold:\n                collected_results[header] = {\"maximum library
      diversity\": values[0]}\n                break\n\n\ndef collect_stats(args):\n\
      \    collected_results = {}\n    if args.trim1:\n        process_trimgalore_report(args.trim1,
      collected_results)\n    if args.trim2:\n        process_trimgalore_report(args.trim2,
      collected_results)\n    process_custom_report(args.bowtie, collected_results,
      \"alignment statistics\", BOWTIE)\n    process_custom_report(args.bamstats,
      ...
      \    process_custom_report(args.macs2, collected_results, \"peak calling statistics\"\
      , MACS2)\n    process_macs2_xls(args.macs2, collected_results, \"peak calling
      statistics\")\n    process_atdp_results(args.atdp, collected_results, \"average
      tag density\")\n    if args.preseq:\n        process_preseq_results(args.preseq,
      collected_results, \"library preparation\")\n    return (collected_results)\n
      ...
      peaks called\",\n                    \"mean peak size\",\n                 \
      \   \"total reads/pairs in treatment\",\n                    \"reads/pairs after
      filtering in treatment\",\n                    \"redundant rate in treatment\"\
      ,\n                    \"fraction of reads in peaks\",\n\n                 \
      \   \"average tag density\",\n                    \"maximum\"]\n\n        if
      collected_data.get(\"adapter trimming statistics\", None):\n            header.extend([\"\
      adapter trimming statistics\",\n                        \"trimming mode\",\n\
      \                        \"adapter sequence\",\n                        \"quality
      phred score cutoff\",\n                        \"minimum required adapter overlap\"\
      ,\n                        \"maximum trimming error rate\",\n              \
      \          \"minimum required read length\",\n                        \"number
      of reads/pairs analysed for length validation\",\n                        \"\
      reads/pairs removed because of length cutoff\",\n                        \"
      fastq1: total reads processed\",\n                        \"fastq1: reads with
      adapters\"])\n            if len(collected_data[\"adapter trimming statistics\"\
      ][\"fastq\"]) > 1:\n                header.extend([\"fastq2: total reads processed\"\
      , \"fastq2: reads with adapters\"])\n\n        output_stream.write(\"\\t\".join(header)+\"\
      \\n\")\n\n        data = [\n                total,\n                mapped,\n\
      \                multimapped,\n                unmapped,\n                filtered,\n\
      \n                \"\",\n                collected_data[\"alignment statistics\"\
      ][\"total reads/pairs processed\"],\n                collected_data[\"alignment
      statistics\"][\"reads/pairs with at least one reported alignment\"],\n     \
      \           collected_data[\"alignment statistics\"].get(\"reads/pairs suppressed
      due to multimapping\", 0),\n                collected_data[\"alignment statistics\"\
      ][\"reads/pairs unmapped\"],\n                \n                \"\",\n    \
      ...
      ],\n                collected_data[\"BAM statistics\"][\"insert size standard
      deviation\"],\n                collected_data[\"BAM statistics\"][\"reads average
      length\"],\n                collected_data[\"BAM statistics\"][\"reads average
      quality\"],\n                collected_data[\"BAM statistics\"][\"reads maximum
      length\"],\n                \n                \"\",\n                collected_data[\"\
      BAM statistics after filtering\"][\"total reads/pairs\"],\n                collected_data[\"\
      BAM statistics after filtering\"][\"reads/pairs mapped\"],\n               \
      \ collected_data[\"BAM statistics after filtering\"][\"reads/pairs unmapped\"\
      ],\n                collected_data[\"BAM statistics after filtering\"][\"reads/pairs
      duplicated\"],\n                collected_data[\"BAM statistics after filtering\"\
      ][\"insert size average\"],\n                collected_data[\"BAM statistics
      after filtering\"][\"insert size standard deviation\"],\n                collected_data[\"\
      BAM statistics after filtering\"][\"reads average length\"],\n             \
      \   collected_data[\"BAM statistics after filtering\"][\"reads average quality\"\
      ],\n                collected_data[\"BAM statistics after filtering\"][\"reads
      maximum length\"],\n                \n                \"\",\n              \
      \  collected_data[\"peak calling statistics\"][\"number of peaks called\"],\n\
      \                collected_data[\"peak calling statistics\"][\"mean peak size\"\
      ],\n                collected_data[\"peak calling statistics\"][\"total reads/pairs
      in treatment\"],\n                collected_data[\"peak calling statistics\"\
      ][\"reads/pairs after filtering in treatment\"],\n                collected_data[\"\
      peak calling statistics\"][\"redundant rate in treatment\"],\n             \
      \   collected_data[\"peak calling statistics\"][\"fraction of reads in peaks\"\
      ],\n\n                \"\",\n                collected_data[\"average tag density\"\
      ][\"maximum\"]]\n\n        if collected_data.get(\"adapter trimming statistics\"\
      , None):\n            data.extend([\"\",\n                        collected_data[\"\
      adapter trimming statistics\"][\"trimming mode\"],\n                       \
      \ collected_data[\"adapter trimming statistics\"][\"adapter sequence\"],\n \
      \                       collected_data[\"adapter trimming statistics\"][\"quality
      phred score cutoff\"],\n                        collected_data[\"adapter trimming
      statistics\"][\"minimum required adapter overlap\"],\n                     \
      \   collected_data[\"adapter trimming statistics\"][\"maximum trimming error
...
label: collect-statistics-chip-seq-trim
