...
doc: "A CWL tool for producing a GCT data file for the morpheus heatmap, and an html
  ...
  the form of gene expression count matrices are processed to output TotalReads and
  Rpkm values per gene.\nAdditionally, TotalReads are used to produce a separate count
  matrix of VST values, and VST z-scores.\nThis data is then integrated into a single
  count matrix (one for scaled, a separate matrix for VST, and another for VST z-scores),
  a row, and a column metadata file.\nThese are used as input to an Rscript that will
  format the 3 files into GCT format for morpheus heatmap generation.\n\n\nPrimary
  Output files:\n- heatmap_TotalReads.html, html of morpheus heatmap with preconfigured
  settings, TotalReads, no data scaling\n- heatmap_vst.html, html of morpheus heatmap
  with preconfigured settings, VST values, no data scaling\n- heatmap_vst_zscore.html,
  html of morpheus heatmap with preconfigured settings, Z-scores of VST values, no
  data scaling\n- heatmap_Rpkm.html, html of morpheus heatmap with preconfigured settings,
  RPKM, no data scaling\n- heatmap_scaled100.html, html of morpheus heatmap with preconfigured
  settings, RPKM, data scaled 0-99, no percentile cutoff\n- heatmap_scaled99.html,
  html of morpheus heatmap with preconfigured settings, RPKM, data scaled 0-99, max
  value set to 99th percentile RPKM value\n- heatmap_scaled95.html, html of morpheus
  heatmap with preconfigured settings, RPKM, data scaled  0-99, max value set to 95th
  percentile RPKM value\n\nSecondary Output files:\n- master_samplesheet_scaled.tsv,
  contains formatted information of the input data and files for raw and scaled heatmaps\n
  - master_samplesheet_vst.tsv, contains formatted information of the input data and
  files for vst normalized heatmaps\n- master_samplesheet_vst_zscore.tsv, contains
  formatted information of the input data and files for vst normalized heatmaps\n\n\
  PARAMS:\n  SECTION 1: general\n  -h\thelp\t\t    show this message\n  -t  INT\t\t\
  \t    number of threads\n  -a\tARRAY\t\t    array of genelist sample names (no commas
  in names)\n  -b  FILE ARRAY\tarray of associated annotation files for each gene
  list from (-c), with header\n  -c  FILE ARRAY\tarray of filtered gene list TSVs
  (must be headerless, columns are: chr, txStart, txEnd, geneID, L2FC, Strand)\n \
  \ -e\tARARY\t\t    array of sample names from RNA-Seq experiments (no commas in
