  ...
  - class: InlineJavascriptRequirement
    expressionLib:
      - var get_output_filename = function() { if (inputs.output_filename) { return
        inputs.output_filename; } return 
        inputs.star_log.location.split('/').slice(-1)[0].replace(/\.*Log\.final\.out$/i,'')+".stat";
        };
      - var get_formatted_output_filename = function() { if (inputs.formatted_output_filename)
        { return inputs.formatted_output_filename; } return 
        inputs.star_log.location.split('/').slice(-1)[0].replace(/\.*Log\.final\.out$/i,'')+"_stats.tsv";
        };
hints:
...
inputs:
  script:
    ...
    inputBinding:
      position: 5
    doc: Python script to get TOTAL, ALIGNED, RIBO, MULTIMAPPED, USED values from
      log files
  star_log:
    type: File
    inputBinding:
