...
doc: "Tool merges input feature files based on the columns provided in --mergeby\n
  ...
  *.tsv - TSV\nOtherwise used CSV by default\n\nThe output file's rows order corresponds
  to the rows order of the first\nCSV/TSV feature file. Output is always saved in
  TSV format.\n\nOutput file includes only rows intersected by column names set in
