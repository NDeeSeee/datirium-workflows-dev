...
inputs:
  ...
  tss_exclusion_zone_size:
    type: int?
    default: 2500
    label: Exclude regions contained within +/- this distance from TSS (skip TSS exclusion
      if 0)
    doc: |
  ...
  rename_png:
    in:
      ...
      target_filename:
        ...
        valueFrom: 
          $(self.location.split('/').slice(-1)[0].split('.').slice(0,-1).join('.')+"_default_s_enhcr.png")
    out:
      - target_file
    run:
      cwlVersion: v1.0
  ...
  bed_to_bigbed:
    in:
      ...
      output_filename:
        source: alignment_file
        valueFrom: 
          $(self.location.split('/').slice(-1)[0].split('.').slice(0,-1).join('.')+"_default_s_enhcr.bb")
    out:
      - bigbed_file
    run:
      cwlVersion: v1.0
      ...
      inputs:
        bed_type:
          ...
          inputBinding:
            ...
            prefix: -type=
            separate: false
          doc: Type of BED file in a form of bedN[+[P]]. By default bed3 to three
            required BED fields
        input_bed:
          type: File
          inputBinding:
  ...
  bed_to_macs:
    ...
    run:
      ...
      baseCommand:
        ...
        - -c
      arguments:
        - cat $0 | grep -v "#" | awk 'BEGIN {print 
          "chr\tstart\tend\tlength\tabs_summit\tpileup\t-log10(pvalue)\tfold_enrichment\t-log10(qvalue)\tname"}
          {print $1"\t"$2"\t"$3"\t"$3-$2+1"\t0\t0\t0\t0\t0\t"$4}' > `basename $0`
      doc: Tool converts `input_file` to the format compatible with the input of iaintersect
        from `assign_genes` step
  assign_genes:
  ...
  add_island_names:
    in:
      ...
      param:
        source: alignment_file
        valueFrom: 
          $(self.location.split('/').slice(-1)[0].split('.').slice(0,-1).join('.')+"_default_s_enhcr.tsv")
    out:
      - output_file
    run:
      cwlVersion: v1.0
      ...
      arguments:
        - echo -e 
          "refseq_id\tgene_id\ttxStart\ttxEnd\tstrand\tchrom\tstart\tend\tlength\tregion\tname\tscore"
          > `basename $2`; cat $0 | grep -v refseq_id | paste - $1 | cut -f 1-9,15,19,20
          >> `basename $2`
