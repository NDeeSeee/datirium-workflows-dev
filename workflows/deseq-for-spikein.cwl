...
inputs:
  ...
  alias_cond_1:
    ...
    doc: Name to be displayed for condition 1, e.g. 'treatment' (letters and numbers
      only)
    sd:layout:
      advanced: true
  alias_cond_2:
    ...
    doc: Name to be displayed for condition 2, e.g. 'control' (letters and numbers
      only)
    sd:layout:
      advanced: true
  cluster_method:
  ...
  center_row:
    type: boolean?
    default: false
    label: Apply mean centering for feature expression prior to running clustering
      by row
    doc: |
...
doc: "# Differential gene expression analysis\n\nThis differential gene expression
  (DGE) analysis takes as input samples from two experimental conditions that have
  been processed with a spike-in normalized RNA-Seq workflow (see list of \"Upstream
  workflows\" at top of file).\n\nThe size factor estimation and application for normalization
  is disabled in this version of the DESeq workflow, otherwise all other aspects are
  the same.\n\nDESeq estimates variance-mean dependence in count data from high-throughput
  sequencing assays, then tests for DGE based on a model which assumes a negative
  binomial distribution of gene expression (aligned read count per gene).\n\nIn this
  pipeline we also use ERCC (External RNA Controls Consortium) synthetic RNA sequences.
  These spike-ins are added into the sample at known concentrations and serve as external
  references to assess technical variability, normalization, and accuracy of quantification.
  DESeq2 uses the expression levels (raw counts) of the ERCC spike-ins to estimate
  the size factor of all samples in the experiment. ERCC spike-ins are especially
  valuable in experiments involving complex samples or when comparing data across
  different platforms or conditions. Most importantly, the use of ERCC controls enable
  the detection of global changes in gene expression, involving a myriad of genes,
  and not just a few selected genes.\n\n### Experimental Setup and Results Interpretation\n
  \nThe workflow design uses as its fold change (FC) calculation: condition 1 (c1,
  e.g. treatment) over condition 2 (c2, e.g. control).\n\nIn other words: `FC == (c1/c2)`\n
  \nTherefore:\n\n- if FC<1 the log2(FC) is <0 (negative), meaning expression in condition1<condition2
  (gene is downregulated in c1)\n- if FC>1 the log2(FC) is >0 (positive), meaning
  expression in condition1>condition2 (gene is upregulated in c1)\n\nIn other words,
  if you have input TREATMENT samples as condition 1, and CONTROL samples as condition
  2, a positive L2FC for a gene indicates that expression of the gene in TREATMENT
  is greater (or upregulated) compared to CONTROL.\n\nNext, threshold the p-adjusted
  values with your FDR (false discovery rate) cutoff to determine if the change may
  be considered significant or not.\n\nIn a typical gene expression analysis experiment,
  DESeq2 calaculates biological variability per gene within each condition is available
  to be incorporated into the model, and resulting p-values are assumed to be \"biological\"\
  . Additionally, DESeq2 fold change is \"shrunk\" to account for sample variability,
  and as Michael Love (DESeq maintainer) puts it, \"it looks at the largest fold changes
  that are not due to low counts and uses these to inform a prior distribution. So
  the large fold changes from genes with lots of statistical information are not shrunk,
  while the imprecise fold changes are shrunk. This allows you to compare all estimated
  LFC across experiments, for example, which is not really feasible without the use
  of a prior\".\n\nIn either case, the null hypothesis (H0) tested is that there are
  no significantly differentially expressed genes between conditions, therefore a
  smaller p-value indicates a lower probability of the H0 occurring by random chance
  and therefore, below a certain threshold (traditionally <0.05), H0 should be rejected.
  Additionally, due to the many thousands of independent hypotheses being tested (each
  gene representing an independent test), the p-values attained by the Wald test are
  adjusted using the Benjamini and Hochberg method by default. These \"padj\" values
  should be used for determination of significance (a reasonable value here would
  be <0.10, i.e. below a 10% FDR).\n\nFurther Analysis: Output from the DESeq workflow
  may be used as input to the GSEA (Gene Set Enrichment Analysis) workflow for identifying
  enriched marker gene sets between conditions.\n\n### DESeq1\n\nSupport for DESeq1
  was dropped in the current version of this pipeline, such that experiment with only
  one replicate per sample cannot be analysed. \n\n### DESeq2\n\nIn comparative high-throughput
  sequencing assays, a fundamental task is the analysis of count data, such as read
  counts per gene in RNA-seq, for evidence of systematic changes across experimental
  conditions. Small replicate numbers, discreteness, large dynamic range and the presence
  of outliers require a suitable statistical approach. [DESeq2](http://www.bioconductor.org/packages/release/bioc/html/DESeq2.html),
  a method for differential analysis of count data, using shrinkage estimation for
  dispersions and fold changes to improve stability and interpretability of estimates.
  ...
  W (2010). “Differential expression analysis for sequence count data.” Genome Biology,
  11, R106. doi: 10.1186/gb-2010-11-10-r106, http://genomebiology.com/2010/11/10/R106/.\n\
  \  - Love MI, Huber W, Anders S (2014). “Moderated estimation of fold change and
  dispersion for RNA-seq data with DESeq2.” Genome Biology, 15, 550. doi: 10.1186/s13059-014-0550-8.\n\
  \  - Lovén J, Orlando DA, Sigova AA, Lin CY, Rahl PB, Burge CB, Levens DL, Lee TI,
  Young RA. Revisiting global gene expression analysis. Cell. 2012 Oct 26;151(3):476-82.
  doi: 10.1016/j.cell.2012.10.012. PMID: 23101621; PMCID: PMC3505597.\n"
sd:version: 100
