...
inputs:
  ...
  numeric_threshold:
    ...
    sd:layout:
      advanced: true
    label: 'A numeric threshold (n) to return top n fraction of peaks based on total
      signal within peaks (default 0.01):'
    sd:localLabel: true
    doc: A numeric threshold n between 0 and 1 returns the top n fraction of peaks
      based on total signal within peaks. Default is 0.01
  promoter_dist:
    type: int?
    default: 1000
    sd:layout:
...
outputs:
  ...
  seacr_relaxed_peaks:
    ...
    doc: Bed file of enriched regions called by seacr relaxed mode (from normalized
      bigwig) in macs2's bed format.
  ...
  seacr_stringent_peaks:
    ...
    doc: Bed file of enriched regions called by seacr stringent mode (from normalized
      bigwig) in macs2's bed format.
  ...
  macs2_called_peaks:
    type: File?
    label: Called peaks
    doc: XLS file of enriched regions called by seacr stringent mode(from normalized
      bigwig) in macs2 output format.
