...
inputs:
  ...
  filtered_list_A:
    type: File
    label: 'Filtered genelist A (important to choose a specific list for relative
      complement):'
    doc: Select list A from filtered differential genelists from DESeq or diffbind
      pipelines
  ...
  filtered_list_B_group:
    ...
    doc: Select 1 or more lists from filtered differential genelists from DESeq or
      diffbind pipelines. These will be used to generate intersections and unions
      among all lists (including list A), and will be applied against list A for the
