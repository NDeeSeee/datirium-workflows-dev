...
requirements:
  ...
  - class: InlineJavascriptRequirement
    expressionLib:
      ...
      - var get_query_column = function(prefix, reduction, resolution) { if (reduction=="RNA")
        { return prefix + "rna_res." + resolution; } else if (reduction=="ATAC") {
        return prefix + "atac_res." + resolution; } else if (reduction=="WNN") { return
        prefix + "wsnn_res." + resolution; } };
sd:upstream:
  sc_tools_sample:
    - sc-rna-cluster.cwl
    - sc-atac-cluster.cwl
...
outputs:
  ...
  umi_mito_spl_ctyp_plot_png:
    ...
    sd:visualPlugins:
      - image:
          tab: QC
          Caption: RNA reads vs mitochondrial % per cell (split by cell type, all
            cells)
  tss_frgm_spl_ctyp_plot_png:
    type: File?
    outputSource: ctype_assign/tss_frgm_spl_ctyp_plot_png
    label: TSS enrichment score vs ATAC fragments in peaks per cell (split by cell
      type, all cells)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          ...
          Caption: TSS enrichment score vs ATAC fragments in peaks per cell (split
            by cell type, all cells)
  rna_atac_cnts_spl_ctyp_plot_png:
    type: File?
    outputSource: ctype_assign/rna_atac_cnts_spl_ctyp_plot_png
    label: RNA reads vs ATAC fragments in peaks per cell (split by cell type, all
      cells)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: QC
          Caption: RNA reads vs ATAC fragments in peaks per cell (split by cell type,
            all cells)
  ...
  umap_gr_ctyp_spl_ph_png:
    ...
    sd:visualPlugins:
      - image:
          tab: Split by cell type
          Caption: UMAP colored by cell type (split by cell cycle phase, optionally
            downsampled)
  ...
  umap_gr_ph_spl_cnd_plot_png:
    ...
    sd:visualPlugins:
      - image:
          tab: Split by group
          Caption: UMAP colored by cell cycle phase (split by grouping condition,
            downsampled)
