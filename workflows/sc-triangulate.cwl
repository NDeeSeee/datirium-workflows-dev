...
requirements:
  ...
  - class: InlineJavascriptRequirement
    expressionLib:
      ...
      - var get_query_column = function(reduction, resolution) { if (reduction=="RNA")
        { return Array.from(split_numbers(resolution), r => "rna_res." + r); } else
        if (reduction=="ATAC") { return Array.from(split_numbers(resolution), r =>
        "atac_res." + r); } else if (reduction=="WNN") { return Array.from(split_numbers(resolution),
        r => "wsnn_res." + r); } };
sd:upstream:
  sc_tools_sample:
    - sc-rna-cluster.cwl
...
inputs:
  ...
  barcodes_data:
    type: File?
    label: Optional TSV/CSV file to prefilter and extend metadata be barcodes. First
