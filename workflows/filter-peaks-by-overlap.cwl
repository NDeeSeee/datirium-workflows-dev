...
inputs:
  ...
  peak_list_A:
    ...
    doc: Choose a sample to use for peak list A, from the ChIP, ATAC, C&R, or diffbind
      workflows.
  ...
  peak_list_B_group:
    ...
    doc: Select 1 or more samples for peak list group B, from the ChIP, ATAC, C&R,
      or diffbind workflows.
...
outputs:
  ...
  iaintersect_input:
    type: File
    label: Set peaks from operator, formatted for input into iaintersect to find nearest
      gene per peak.
    doc: 'Regions of interest formatted as headered BED file with [chrom start end
      length abssummit pileup log1-p foldenrich log10q region]. NOTE: pileup, log1-p,
      foldenrich, and log10q are padded with 0s due to non-carrythrough after set
      operations.'
