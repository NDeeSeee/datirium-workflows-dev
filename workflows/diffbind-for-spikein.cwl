...
inputs:
  ...
  peak_files_cond_1:
    type: File[]
    label: Biological condition 1 samples. Minimum 2 samples
    doc: XLS peak files for condition 1 from MACS2. Minimim 2 files. Order corresponds
      to read_files_cond_1
    sd:upstreamSource: first_biological_condition/macs2_called_peaks
  peak_files_cond_2:
    type: File[]
    label: Biological condition 2 samples. Minimum 2 samples
    doc: XLS peak files for condition 2 from MACS2. Minimim 2 files. Order corresponds
      to read_files_cond_2
  ...
  sample_names_cond_1:
    ...
    doc: Aliases for biological condition 1 samples to make the legend for generated
      plots. Order corresponds to the read_files_cond_1
  ...
  sample_names_cond_2:
    ...
    doc: Aliases for biological condition 2 samples to make the legend for generated
      plots. Order corresponds to the read_files_cond_2
  ...
  blocked_file:
    type: File?
    label: Blocking attribute headerless TSV/CSV file for multi-factor analysis with
      columns to set name and group. If this inputs is set, blocking attributes above
      are ignored
    doc: |
  ...
  promoter_dist:
    ...
    doc: 'Max distance from gene TSS (in both direction) overlapping which the peak
      will be assigned to the promoter region. Default: 1000 bp'
    sd:layout:
      advanced: true
  upstream_dist:
    type: int?
    ...
    doc: 'Max distance from the promoter (only in upstream direction) overlapping
...
outputs:
  ...
  diffbind_db_sites_correlation_heatmap:
    type: File?
    label: Normalized counts correlation heatmap for significantly differentially
      bound sites
    ...
    sd:visualPlugins:
      - image:
          tab: Plots
          Caption: Normalized counts correlation heatmap for significantly differentially
            bound sites
  ...
  diffbind_boxplot_plot:
    type: File?
    label: Box plots of read distributions for significantly differentially bound
      sites
    ...
    sd:visualPlugins:
      - image:
          tab: Plots
          Caption: Box plots of read distributions for significantly differentially
            bound sites
...
label: DiffBind spike-in - Differential Binding Analysis of ChIP-Seq or CUTß&RUN/Tag
  Peak Data with spike-in
doc: |-
