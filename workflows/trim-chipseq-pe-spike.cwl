...
outputs:
  ...
  bambai_pair_spikein:
    ...
    doc: Coordinate sorted BAM file and BAI index file for spike-in, used for normalization
      in diffbind downstream
...
doc: "A basic analysis workflow for paired-end ChIP-Seq experiments with a spike-in
  control. These sequencing library prep methods are chromatin mapping technologies
  ...
  distribution (of the alignment pileup).\n\n### __Inputs__\n*General Info (required\\
  *):*\n- Experiment short name/Alias* - a unique name for the sample (e.g. what was
  used on tubes while processing it)\n- Cells* - sample cell type or organism name\n
  - Conditions* - experimental condition name\n- Catalog # - catalog number for cells
  from vender/supplier\n- Primary [genome index](https://scidap.com/tutorials/basic/genome-indices)
  for peak calling* - preprocessed genome index of sample organism for primary alignment
  ...
  for normalization, used to scale the read count pileups from the primary alignment
  used for peak calling. After normalization, sample pileups/peak may then be appropriately
  compared to one another assuming an equal use of spike-in material during library
