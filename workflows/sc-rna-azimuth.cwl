...
requirements:
  ...
  - class: InlineJavascriptRequirement
    expressionLib:
      ...
      - var get_source_column = function(prefix, reduction, resolution) { if (reduction
        == "RNA" && resolution != null) { return prefix + "rna_res." + resolution;
        } else if (reduction == "WNN" && resolution != null) { return prefix + "wsnn_res."
        + resolution; } else { return null; } };
sd:upstream:
  sc_tools_sample:
    - sc-rna-filter.cwl
...
inputs:
  ...
  reference_data_rds:
    type: File?
    label: Reference Single-cell Analysis with Assigned Cell Types (for a custom reference
      attach files below)
    doc: |
  ...
  export_loupe_data:
    type: boolean?
    default: false
    label: Save raw counts to Loupe file. I confirm that data is generated by 10x
      technology and accept the EULA available at https://10xgen.com/EULA
    doc: |
...
outputs:
  ...
  qc_mtrcs_dnst_gr_ctyp_plot_png:
    ...
    sd:visualPlugins:
      - image:
          tab: QC
          Caption: Distribution of QC metrics per cell colored by cell type (all query
            cells)
  umap_cnf_plot_png:
    type: File?
    ...
    sd:visualPlugins:
      - image:
          tab: QC
          Caption: Projected UMAP colored by prediction confidence score (all query
            cells)
  umap_map_plot_png:
    type: File?
  ...
  gene_umi_gr_cnf_spl_ctyp_plot_png:
    ...
    label: Genes vs RNA reads per cell colored by prediction confidence score (split
      by cell type, all query cells)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: QC
          Caption: Genes vs RNA reads per cell colored by prediction confidence score
            (split by cell type, all query cells)
  gene_umi_gr_map_spl_ctyp_plot_png:
    type: File?
    outputSource: rna_azimuth/gene_umi_gr_map_spl_ctyp_plot_png
    label: Genes vs RNA reads per cell colored by prediction mapping score (split
      by cell type, all query cells)
    doc: |
      Genes vs RNA reads per cell.
      All query cells; split by cell type;
    ...
    sd:visualPlugins:
      - image:
          tab: QC
          Caption: Genes vs RNA reads per cell colored by prediction mapping score
            (split by cell type, all query cells)
  umi_mito_gr_cnf_spl_ctyp_plot_png:
    type: File?
    outputSource: rna_azimuth/umi_mito_gr_cnf_spl_ctyp_plot_png
    label: RNA reads vs mitochondrial % per cell colored by prediction confidence
      score (split by cell type, all query cells)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: QC
          Caption: RNA reads vs mitochondrial % per cell colored by prediction confidence
            score (split by cell type, all query cells)
  umi_mito_gr_map_spl_ctyp_plot_png:
    type: File?
    outputSource: rna_azimuth/umi_mito_gr_map_spl_ctyp_plot_png
    label: RNA reads vs mitochondrial % per cell colored by prediction mapping score
      (split by cell type, all query cells)
    doc: |
      RNA reads vs mitochondrial % per cell.
      All query cells; split by cell type;
    ...
    sd:visualPlugins:
      - image:
          tab: QC
          Caption: RNA reads vs mitochondrial % per cell colored by prediction mapping
            score (split by cell type, all query cells)
  tss_frgm_gr_cnf_spl_ctyp_plot_png:
    type: File?
    outputSource: rna_azimuth/tss_frgm_gr_cnf_spl_ctyp_plot_png
    label: TSS enrichment score vs ATAC fragments in peaks per cell colored by prediction
      confidence score (split by cell type, all query cells)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: QC
          Caption: TSS enrichment score vs ATAC fragments in peaks per cell colored
            by prediction confidence score (split by cell type, all query cells)
  tss_frgm_gr_map_spl_ctyp_plot_png:
    type: File?
    outputSource: rna_azimuth/tss_frgm_gr_map_spl_ctyp_plot_png
    label: TSS enrichment score vs ATAC fragments in peaks per cell colored by prediction
      mapping score (split by cell type, all query cells)
    doc: |
      TSS enrichment score vs ATAC fragments
      in peaks per cell.
      All query cells; split by cell type;
    ...
    sd:visualPlugins:
      - image:
          tab: QC
          Caption: TSS enrichment score vs ATAC fragments in peaks per cell colored
            by prediction mapping score (split by cell type, all query cells)
  rna_atac_cnts_gr_cnf_spl_ctyp_plot_png:
    type: File?
    outputSource: rna_azimuth/rna_atac_cnts_gr_cnf_spl_ctyp_plot_png
    label: RNA reads vs ATAC fragments in peaks per cell colored by prediction confidence
      score (split by cell type, all query cells)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: QC
          Caption: RNA reads vs ATAC fragments in peaks per cell colored by prediction
            confidence score (split by cell type, all query cells)
  rna_atac_cnts_gr_map_spl_ctyp_plot_png:
    type: File?
    outputSource: rna_azimuth/rna_atac_cnts_gr_map_spl_ctyp_plot_png
    label: RNA reads vs ATAC fragments in peaks per cell colored by prediction mapping
      score (split by cell type, all query cells)
    doc: |
      RNA reads vs ATAC fragments in peaks per cell.
      All query cells; split by cell type; colored
  ...
  umap_gr_ctyp_spl_ph_png:
    ...
    label: Projected UMAP colored by cell type (split by cell cycle phase, optionally
      downsampled filtered query cells)
    doc: |
      Projected UMAP colored by cell type.
    ...
    sd:visualPlugins:
      - image:
          tab: Split by cell type
          Caption: Projected UMAP colored by cell type (split by cell cycle phase,
            optionally downsampled filtered query cells)
  cmp_gr_ph_spl_ctyp_png:
    type: File?
    outputSource: rna_azimuth/cmp_gr_ph_spl_ctyp_png
    label: Composition plot colored by cell cycle phase (split by cell type, optionally
      downsampled filtered query cells)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Split by cell type
          Caption: Composition plot colored by cell cycle phase (split by cell type,
            optionally downsampled filtered query cells)
  umap_gr_ctyp_spl_idnt_plot_png:
    type: File?
    outputSource: rna_azimuth/umap_gr_ctyp_spl_idnt_plot_png
    label: Projected UMAP colored by cell type (split by dataset, downsampled filtered
      query cells)
    doc: |
      Projected UMAP colored by cell type.
    ...
    sd:visualPlugins:
      - image:
          ...
          Caption: Projected UMAP colored by cell type (split by dataset, downsampled
            filtered query cells)
  cmp_gr_ctyp_spl_idnt_plot_png:
    type: File?
    outputSource: rna_azimuth/cmp_gr_ctyp_spl_idnt_plot_png
    label: Composition plot colored by cell type (split by dataset, downsampled filtered
      query cells)
    doc: |
  ...
  umap_gr_ph_spl_idnt_plot_png:
    ...
    label: Projected UMAP colored by cell cycle phase (split by dataset, downsampled
      filtered query cells)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Split by dataset
          Caption: Projected UMAP colored by cell cycle phase (split by dataset, downsampled
            filtered query cells)
  cmp_gr_ph_spl_idnt_plot_png:
    ...
    sd:visualPlugins:
      - image:
          tab: Split by dataset
          Caption: Composition plot colored by cell cycle phase (split by dataset,
            downsampled filtered query cells)
  umap_gr_ctyp_spl_cnd_plot_png:
    type: File?
    outputSource: rna_azimuth/umap_gr_ctyp_spl_cnd_plot_png
    label: Projected UMAP colored by cell type (split by grouping condition, downsampled
      filtered query cells)
    doc: |
      Projected UMAP colored by cell type.
    ...
    sd:visualPlugins:
      - image:
          tab: Split by group
          Caption: Projected UMAP colored by cell type (split by grouping condition,
            downsampled filtered query cells)
  cmp_gr_ctyp_spl_cnd_plot_png:
    ...
    sd:visualPlugins:
      - image:
          tab: Split by group
          Caption: Composition plot colored by cell type (split by grouping condition,
            downsampled filtered query cells)
  umap_gr_ph_spl_cnd_plot_png:
    type: File?
    outputSource: rna_azimuth/umap_gr_ph_spl_cnd_plot_png
    label: Projected UMAP colored by cell cycle phase (split by grouping condition,
      downsampled filtered query cells)
    doc: |
      Projected UMAP colored by cell cycle phase.
      Filtered query cells; split by grouping
    ...
    sd:visualPlugins:
      - image:
          tab: Split by group
          Caption: Projected UMAP colored by cell cycle phase (split by grouping condition,
            downsampled filtered query cells)
  cmp_gr_ph_spl_cnd_plot_png:
    type: File?
    outputSource: rna_azimuth/cmp_gr_ph_spl_cnd_plot_png
    label: Composition plot colored by cell cycle phase (split by grouping condition,
      downsampled filtered query cells)
    doc: |
      Composition plot colored by cell cycle phase.
    ...
    sd:visualPlugins:
      - image:
          tab: Split by group
          Caption: Composition plot colored by cell cycle phase (split by grouping
            condition, downsampled filtered query cells)
  xpr_avg_plot_png:
    type: File?
  ...
  xpr_per_cell_plot_png:
    ...
    sd:visualPlugins:
      - image:
          tab: Genes of interest (expression)
          Caption: Projected UMAP colored by gene expression (per gene, filtered query
            cells)
  cvrg_plot_png:
    type:
      - 'null'
  ...
  gse_per_cell_plot_png:
    ...
    sd:visualPlugins:
      - image:
          ...
          Caption: Projected UMAP colored by gene set expression score (all query
            cells)
  gse_avg_plot_png:
    type: File?
