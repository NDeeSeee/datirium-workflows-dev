...
outputs:
  umap_cl_freq_spl_ch_plot_png:
    ...
    label: UMAP colored by clonotype frequency (split by chain, filtered by minimum
      frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: UMAP
          Caption: UMAP colored by clonotype frequency (split by chain, filtered by
            minimum frequency)
  hmst_gr_idnt_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/hmst_gr_idnt_spl_ch_plot_png
    label: Proportion of clonotype frequencies per dataset (split by chain, not filtered
    ...
    sd:visualPlugins:
      - image:
          ...
          Caption: Proportion of clonotype frequencies per dataset (split by chain,
            not filtered by minimum frequency)
  dvrs_gr_idnt_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/dvrs_gr_idnt_spl_ch_plot_png
    label: Diversity of clonotypes per dataset (split by chain, not filtered by minimum
      frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by dataset
          Caption: Diversity of clonotypes per dataset (split by chain, not filtered
            by minimum frequency)
  vrlp_gr_idnt_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/vrlp_gr_idnt_spl_ch_plot_png
    label: Overlap of clonotypes between datasets (split by chain, filtered by minimum
      frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by dataset
          Caption: Overlap of clonotypes between datasets (split by chain, filtered
            by minimum frequency)
  allu_gr_idnt_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/allu_gr_idnt_spl_ch_plot_png
    label: Proportion of top shared clonotypes between datasets (split by chain, filtered
      by minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by dataset
          Caption: Proportion of top shared clonotypes between datasets (split by
            chain, filtered by minimum frequency)
  cl_qnt_gr_idnt_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/cl_qnt_gr_idnt_spl_ch_plot_png
    label: Percentage of unique clonotypes per dataset (split by chain, filtered by
      minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by dataset
          Caption: Percentage of unique clonotypes per dataset (split by chain, filtered
            by minimum frequency)
  gene_gr_idnt_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/gene_gr_idnt_spl_ch_plot_png
    label: Distribution of gene usage per dataset (split by chain, filtered by minimum
      frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by dataset
          Caption: Distribution of gene usage per dataset (split by chain, filtered
            by minimum frequency)
  cl_dnst_gr_idnt_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/cl_dnst_gr_idnt_spl_ch_plot_png
    label: Distribution of clonotype frequencies per dataset (split by chain, filtered
      by minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by dataset
          Caption: Distribution of clonotype frequencies per dataset (split by chain,
            filtered by minimum frequency)
  hmst_gr_dnr_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/hmst_gr_dnr_spl_ch_plot_png
    label: Proportion of clonotype frequencies per donor (split by chain, not filtered
      by minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          ...
          Caption: Proportion of clonotype frequencies per donor (split by chain,
            not filtered by minimum frequency)
  dvrs_gr_dnr_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/dvrs_gr_dnr_spl_ch_plot_png
    label: Diversity of clonotypes per donor (split by chain, not filtered by minimum
      frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by donor
          Caption: Diversity of clonotypes per donor (split by chain, not filtered
            by minimum frequency)
  vrlp_gr_dnr_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/vrlp_gr_dnr_spl_ch_plot_png
    label: Overlap of clonotypes between donors (split by chain, filtered by minimum
      frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by donor
          Caption: Overlap of clonotypes between donors (split by chain, filtered
            by minimum frequency)
  allu_gr_dnr_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/allu_gr_dnr_spl_ch_plot_png
    label: Proportion of top shared clonotypes between donors (split by chain, filtered
      by minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by donor
          Caption: Proportion of top shared clonotypes between donors (split by chain,
            filtered by minimum frequency)
  cl_qnt_gr_dnr_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/cl_qnt_gr_dnr_spl_ch_plot_png
    label: Percentage of unique clonotypes per donor (split by chain, filtered by
      minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by donor
          Caption: Percentage of unique clonotypes per donor (split by chain, filtered
            by minimum frequency)
  gene_gr_dnr_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/gene_gr_dnr_spl_ch_plot_png
    label: Distribution of gene usage per donor (split by chain, filtered by minimum
      frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by donor
          Caption: Distribution of gene usage per donor (split by chain, filtered
            by minimum frequency)
  cl_dnst_gr_dnr_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/cl_dnst_gr_dnr_spl_ch_plot_png
    label: Distribution of clonotype frequencies per donor (split by chain, filtered
      by minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by donor
          Caption: Distribution of clonotype frequencies per donor (split by chain,
            filtered by minimum frequency)
  hmst_gr_cnd_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/hmst_gr_cnd_spl_ch_plot_png
    label: Proportion of clonotype frequencies per grouping condition (split by chain,
      not filtered by minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          ...
          Caption: Proportion of clonotype frequencies per grouping condition (split
            by chain, not filtered by minimum frequency)
  dvrs_gr_cnd_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/dvrs_gr_cnd_spl_ch_plot_png
    label: Diversity of clonotypes per grouping condition (split by chain, not filtered
      by minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by condition
          Caption: Diversity of clonotypes per grouping condition (split by chain,
            not filtered by minimum frequency)
  vrlp_gr_cnd_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/vrlp_gr_cnd_spl_ch_plot_png
    label: Overlap of clonotypes between grouping conditions (split by chain, filtered
      by minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by condition
          Caption: Overlap of clonotypes between grouping conditions (split by chain,
            filtered by minimum frequency)
  allu_gr_cnd_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/allu_gr_cnd_spl_ch_plot_png
    label: Proportion of top shared clonotypes between grouping conditions (split
      by chain, filtered by minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by condition
          Caption: Proportion of top shared clonotypes between grouping conditions
            (split by chain, filtered by minimum frequency)
  cl_qnt_gr_cnd_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/cl_qnt_gr_cnd_spl_ch_plot_png
    label: Percentage of unique clonotypes per grouping condition (split by chain,
      filtered by minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by condition
          Caption: Percentage of unique clonotypes per grouping condition (split by
            chain, filtered by minimum frequency)
  gene_gr_cnd_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/gene_gr_cnd_spl_ch_plot_png
    label: Distribution of gene usage per grouping condition (split by chain, filtered
      by minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by condition
          Caption: Distribution of gene usage per grouping condition (split by chain,
            filtered by minimum frequency)
  cl_dnst_gr_cnd_spl_ch_plot_png:
    type: File?
    outputSource: vdj_profile/cl_dnst_gr_cnd_spl_ch_plot_png
    label: Distribution of clonotype frequencies per grouping condition (split by
      chain, filtered by minimum frequency)
    doc: |
    ...
    sd:visualPlugins:
      - image:
          tab: Grouped by condition
          Caption: Distribution of clonotype frequencies per grouping condition (split
            by chain, filtered by minimum frequency)
  clonotypes_data_tsv:
    type: File?
